...
The tool clarifies the (agent-generated or old) code's intent, reduces non-alphanumeric noise, and uses built-in .NET libraries more efficiently.
...
fsharp-refactor Your.fsproj --dry-run --api-changes
```
...
```bash
fsharp-refactor Your.fsproj --api-changes
```
...
| Kind | | Count |
|---|---|---|
| `correctness` | The code does something other than what it looks like it does: a race, a swallowed exception, a disposable that leaks, a comparison that never holds | 60 |
...
Open an F# file containing e.g. `match x with | true -> 1 | false -> 2` 
- a hint appears offering `if x then 1 else 2`.
