...
| FR0167 | Performance | v | | | `for c in s.ToCharArray() do`, `Array.exists Char.IsDigit (s.ToCharArray())`, `s.ToCharArray() |> Array.iter f` | `for c in s do`; `String.exists Char.IsDigit s`, `s |> String.iter f` (editor: a null string throws before and reads as empty after) ||> String.iter f` |
...
A pure binding inside a `for`/`while`/collection lambda that depends on nothing the loop changes is re-evaluated every iteration; the fix hoists it above the loop, keeping the `#if` it was written under (the directive opens its own line above the anchor). Hoisting moves *n* evaluations to one (and an empty loop still runs it once), so "pure" is a whitelist, not a guess: constants, identifiers, tuples, list literals and core operators that are total on their operands — no calls, no property reads, no `|>`/`>>`/`!`/`:=`, and `/`/`%` only by a non-zero literal. An **array** literal stays put where a list hoists: it is a fresh mutable buffer per iteration, and one shared buffer is not the same program. A `let mutable` the binding reads stands the rule down unless nothing the loop's statement calls can assign it (a local one included — since F# 4.0 a closure captures it as a ref cell)
