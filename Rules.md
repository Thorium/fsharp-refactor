...
| FR0167 | Performance | v | | | `for c in s.ToCharArray() do`, `Array.exists Char.IsDigit (s.ToCharArray())`, `s.ToCharArray() \|> Array.iter f` | `for c in s do`; `String.exists Char.IsDigit (nonNull s)`, `nonNull s \|> String.iter f` (FSharp.Core 9+; below it the bare `String.*` twin, editor only) |
