...
| FR0015 | Performance | v | | | `Regex.IsMatch(s, "^abc")`, `Regex.Match(s, "abc").Success`, `Regex.Matches(s, "abc").Count > 0`, `Regex.Split(s, "abc")` | `s.StartsWith("abc", StringComparison.Ordinal)`, `s.Contains "abc"`, `s.Split([\| "abc" \|], StringSplitOptions.None)` |
...
| FR0041 | Performance | v | | | `ids \|> Array.contains v` (int[]/int64[]) | `System.Linq.Enumerable.Contains(ids, v)` (sum/average/min/max: note) |
...
| FR0055 | Correctness | v | | | `try work () with _ -> ()`, `try Int32.Parse s with :? FormatException -> 0` | `if x = 0 then 0 else a / x`, `match Int32.TryParse s with ...`, an IO-only catch, or a log line in the file's idiom (editor) |
...
| FR0106 | Performance | v | | | `Int32.Parse(s.Substring(6, 5))`, `sb.Append(s.Substring(6, 5))`, `writer.Write(s.Substring 6)` | `Int32.Parse(s.AsSpan(6, 5))`, `sb.Append(s.AsSpan(6, 5))`, `writer.Write(s.AsSpan 6)` |
...
| FR0118 | Correctness | v | | | `let! s = client.GetStringAsync(url) // ct in scope`, a `while` loop that never mentions `ct` | `let! s = client.GetStringAsync(url, ct)`; the loop is a note |
...
| FR0123 | Correctness | v | | | `Monitor.Enter gate; try body () finally Monitor.Exit gate`, `sem.Wait()` with no `finally sem.Release()` | `lock gate (fun () -> body ())`; the statements up to `sem.Release()` under `try ... finally` |
...
| FR0154 | Correctness | v | | | `match xs.TryGetValue key with<br>\| true, x -> x<br>\| false, _ -><br>    let res = compute ()<br>    xs.[key] <- res<br>    res` on a `ConcurrentDictionary` | `match xs.TryGetValue key with<br>\| true, x -> x<br>\| false, _ -> xs.GetOrAdd(key, fun _ -> compute ())` |
| FR0155 | Performance | v | v | | `type internal Node(v: int) = ...` that nothing in the project inherits, with a `Node[]` or a `:? Node` somewhere in the project | `[<Sealed>]` above the type |
| FR0156 | Idiom | v | | | `let acc = ResizeArray<T>()` filled by `for` loops whose `acc.Add e` are the only thing the loops do with it, then `List.ofSeq acc`; `let mutable xs = []` fed by `xs <- xs @ [ e ]` (`List.append`, or `e :: xs` read through `List.rev`) and only read after | `let acc = [ for ... do ... e ... ]` with the loops moved inside and each `Add` argument or appended element as a yield; `List.ofSeq acc` and `List.rev xs` become the bare name |
| FR0157 | Idiom | v | | | `let f (region: string) = match region with "eu" -> 1 | "uk" -> 2 | _ -> failwith "?"` where every call site passes one of those literals, or a function whose every exit is one (`Error "not found"` included) feeds the match | `[<RequireQualifiedAccess>] type Region = | Eu | Uk` with `override this.ToString()` returning the original text, the parameter typed `Region`, the arms and the call sites spelling `Region.Eu`, the dead wildcard gone; an exported function keeps its `string` signature behind a private twin and an `OfString` / `Result.mapError string` wrapper under a TODO || "uk" -> 2 \| _ -> failwith "?"` where every call site passes one of those literals (or a function whose every exit is one feeds the match) | `[<RequireQualifiedAccess>] type Region = \| Eu \| Uk` with `override this.ToString()` returning the original text, the parameter typed `Region`, the arms and the call sites spelling `Region.Eu`, and the dead wildcard gone |
| FR0158 | Idiom | v | | | `let mutable line = 0` then `while line < max && blank line do line <- line + 1` (or `line <- line - 1`) | `let rec advanceLine line = if line < max && blank line then advanceLine (line + 1) else line` and `let line = advanceLine 0` (`retreatLine` for the countdown) |
| FR0159 | Correctness | v | | | `float (sum / count)` with integer operands | `float sum / float count` (editor; a literal operand or a scaled product is noted only with `"all": true`) |
| FR0160 | Correctness | v | | | `with ex -> raise (ConfigException("bad config"))`, `with _ -> failwith "bad config"` | `with ex -> raise (ConfigException("bad config", ex))` where the type has the (…, inner) constructor; the `failwith` is a note |
| FR0161 | Correctness | v | | | `holder.Counter.Bump()` where `Counter` is a property returning a `[<Struct>]` whose `Bump` assigns a field | — |
| FR0162 | Correctness | v | | | `let mutable private cache = None` then `if cache.IsNone then cache <- Some (build ())` | — |
| FR0163 | Correctness | v | | | `new Timer(tick, null, 0, 1000) \|> ignore`, `let t = new Timer(...)` never mentioned again (System.Threading) | — |
| FR0164 | Correctness | v | | | `for x in items do if stale x then items.Remove x \|> ignore` | `for x in Array.ofSeq items do ...` |
| FR0165 | Correctness | v | | | `DateTime.Now > startedUtc` where `startedUtc = DateTime.UtcNow`, `DateTime.UtcNow - DateTime.Today` | — |
| FR0166 | Performance | v | | | `s.Substring(0, 6) = "ORDER-"`, `s[..5] = "ORDER-"`, `s[s.Length - 3 ..] <> "MED"` | `s.StartsWith("ORDER-", StringComparison.Ordinal)`, `not (s.EndsWith("MED", StringComparison.Ordinal))` (a slice, or a Substring under a `.Length` guard; a bare Substring editor-only) |
| FR0167 | Performance | v | | | `for c in s.ToCharArray() do`, `Array.exists Char.IsDigit (s.ToCharArray())`, `s.ToCharArray() \|> Array.iter f` | `for c in s do`; `String.exists Char.IsDigit (FSharp.Core.Operators.nonNull s)`, `FSharp.Core.Operators.nonNull s \|> String.iter f` (FSharp.Core 9+; below it the bare `String.*` twin, editor only) |
| FR0168 | Performance | v | | | `try Int32.Parse s with _ -> 0`, `try Some (Int32.Parse s) with :? FormatException -> None` | `match Int32.TryParse s with | true, v -> v | false, _ -> 0`, `... | true, v -> Some v | false, _ -> None` (a catch narrower than what TryParse answers false to is a note) |
...
| FR0174 | Performance | v | | | `db.Orders.ToList().Where(fun o -> o.Total > 0).Select(fun o -> o.Id)` | `db.Orders.Where(fun o -> o.Total > 0).Select(fun o -> o.Id).ToList()` |
...
Literal regex patterns → `StartsWith`/`Contains`: `Regex.IsMatch(s, "abc")`, `Regex.Match(s, "abc").Success` (the Match object built only to be asked) and the count tests `Regex.Matches(s, "abc").Count > 0` / `<> 0` / `>= 1` (the literal on either side) → `s.Contains "abc"`, and `= 0` / `< 1` / `<= 0` → `not (s.Contains "abc")` — every match found and counted to answer whether there was one; a `^`-anchored pattern → `s.StartsWith("abc", StringComparison.Ordinal)` (the regex compared ordinally, `Contains(string)` does too, `StartsWith(string)` would not); a trailing `$` keeps the engine (it also matches before a final newline), and so does `.Count` compared with anything else or standing alone (C#'s twin CR0108 spells that `s.AsSpan().Count("abc")`, a .NET 8 API this parse-only rule cannot prove). A literal-pattern `Regex.Split(s, "ab")` → `s.Split([| "ab" |], StringSplitOptions.None)`, the overload every framework has, the same result (no capture group, so no captured pieces; empty entries kept), declined for an anchor, a metacharacter or a RegexOptions argument. A literal-pattern `Regex.Replace(s, "abcd", "x")` → `s.Replace("abcd", "x")` (declined wherever the two differ, each case measured: a `$` in the replacement is substitution syntax to Regex and plain text to String, an empty pattern makes String.Replace throw where Regex inserts between every character, an anchor is meaning Replace cannot carry, and the RegexOptions / MatchEvaluator overloads are different operations); static `Regex` calls inside loops — `for`/`while`, or a lambda handed to a `List`/`Seq`/`Array` function, which runs once per element — are hoisted to a `let private xRegex = Regex "..."` module binding (advice-only when the `open` is missing or the name is taken; a call under `#if` hoists under the same `#if`; the binding lands above the declaration's doc comment, not between it and the `let`); a `Regex` CONSTRUCTED with a literal pattern inside such a loop — `Regex("lit")`, `new Regex("lit", RegexOptions.IgnoreCase ||| RegexOptions.Multiline)`, the qualified spelling too — is hoisted the same way, source text and all, and only the construction is replaced by the hoisted name: `let regex = Regex(";([a-z]*)=")` followed by `regex.Split(v)` (FSharp.Analyzers.SDK's `expandMultiProperties`, hoisted by hand there) becomes `let regex = azAZRegex` with the `Split` untouched; declined for a non-literal pattern, options naming a local, or a multi-line construction, in which case FR0037 keeps its note. A site in a plain FUNCTION body — no loop above it — is hoisted too, and that half is the knob `{ "FR0015": { "perCall": false } }` (default true): a loop re-parses per element, while a function body costs one lookup in the runtime's regex cache per call and a re-parse only once that cache (fifteen patterns) turns over, so it is the smaller and less certain win — kept on because a function is far likelier to be called many times than a module is to be initialised with its hoisted binding never read, and F# builds a module's bindings on first access, so one nothing reaches is never built. Turning it off leaves the loop hoists and hands the per-call constructions back to FR0037's note

### FR0016 — performance

Small value-type-only unions → `[<Struct>]` (perf: no heap allocation per value) Edits the companion .fsi in step. Stands down where the struct would cost more than the class (CR0081's line): the cases' fields over 32 bytes inline, or a value of the union the file boxes — `box`, `:> obj`, an interface cast, `lock`, `ReferenceEquals`, `Unchecked.defaultof`, a parameter typed `obj`, `string`/`hash`, a `%A`/`%O` hole — where the allocation comes straight back (typed; parse-only runs apply the size cap alone)
...
N+1 note (no fix): a `for` over an `IQueryable` nested inside another loop executes one database query per outer iteration; typed-gated so in-memory sequences never fire, and an outer loop batched with `chunkBySize` suppresses the note - whether the chunking sits in the loop header or was bound to a `let` first, which is how it is usually written

### FR0029 — performance

Task state-machine advice (FS3511 itself is emitted at codegen, invisible to analyzers): a `let rec` in a resumable `task { }` body is flagged always (definite dynamic-fallback producer); oversized tasks (≥8 awaits or ≥60 lines) get the shrinking moves, three of them as automatic fixes — plain leading `let`s hoist above the builder (caveat in the message: a throw there then surfaces at the call instead of faulting the Task), a body that IS an if/else with at least one awaiting arm splits into per-branch tasks (arms cut as line regions between the `then`/`else`/`}` anchors, so their comments travel; a synchronous arm becomes a trivially static task), a long non-awaiting tail wraps into a local function inside the CE (a nested function's body is not resumable code, and closures capture the CE locals — no parameters, no annotations), and, outside the size gate because it costs nothing to take, a closing branch whose every leaf `return`s hoists that one keyword in front of the whole branch — the branch stays where it is and gains an indent level, so the builder is handed a value once instead of once per arm (declined when the branch itself awaits, when it spans a `#if`, and when a directive block opens on the line below it, where another configuration may hold arms this one cannot see). The tail extraction waits for the COMPILER. FS3511 is emitted at codegen where no analyzer can see it, but the apply tool builds before it rewrites and the warning carries the builder's own position, so those lines reach the rule and `runTail` is offered where the dynamic fallback is REAL. Absent that warning the bar is `tailLines` (default 40) — a shape unwieldy on its own merits, since `runTail` names no business idea and a smaller resumable body is not a goal in itself, only FS3511 makes it one. Below the bar the rule says nothing at all: a quickfix or silence, never a note about code with no problem. In the editor, where nothing compiled, only the `tailLines` bar applies. `hoistReturnOnAsync` (default false) extends ONLY the return hoist to `async { }` — worth ~27% less generated IL on a twelve-arm branch but identical on time and allocation, and async has no resumable state machine, so none of the rest of this rule applies there. What remains is advice: elif chains, tails referencing prefix `let!` bindings or foreign local mutables. The non-awaiting tail is measured structurally — the statement suffix after the last await on the spine or in the branch holding it; handlers, `finally` bodies and re-awaiting loop bodies never count, a `return!` is an await, and the note is anchored on the first tail statement (a tail inside a branch is advice only)
...
`List/Array/Seq.contains x ys` inside a loop — or inside a callback given to a collection function — scans `ys` linearly per iteration. When `ys` is a startup-built module binding — immutable, unshadowed, never reassigned — the FIX converts: when EVERY use of the name is one of the probes and the binding is a list/array/`seq` literal, the binding itself becomes the set (`|> Set.ofList`/`ofArray`/`ofSeq` — no companion, still immutable, `Set`'s own `.Contains` takes the probes, measured 2.5× over the list scan even at five elements; a `Set` compares where `List.contains` asked `=`, and NaN compares equal to itself, so an element that can carry a float — a float, or a tuple, record or union holding one — converts only when every element of the literal is a written-out constant, none a NaN: no `nan`, `Double.NaN`, `infinity - infinity` or named value); when other uses pin the binding's type, a private HashSet companion lands beside it instead (built once, `open`-aware spelling) and every probe converts together. Otherwise the note recommends the same by hand, worthwhile only for long loops over more than a handful of elements, measured with the build cost charged (probing the loop variable itself never fires)
...
`Array.sum/average/min/max/contains` on `int[]`/`int64[]` is a scalar loop; on .NET 8+ System.Linq's `Sum()`/`Average()`/`Min()`/`Max()`/`Contains()` are SIMD-vectorized (`Contains` measured ~5x at 1000 elements, ~6x at 100k). `Array.contains v arr` is exact on these types and gets a fix a sweep applies: `System.Linq.Enumerable.Contains(arr, v)` (`Enumerable.Contains(arr, v)` under `open System.Linq`), piped form too — int equality agrees with `EqualityComparer.Default` and both throw ArgumentNullException on a null array; guards: FSharp.Core's Array module, single line, and in the direct form a constant or a name as the probed value when the array is a dotted path (the direct form read the value first); `Seq.contains` over an array is FR0139's fix. The aggregations stay notes: LINQ `Sum` throws on overflow where `Array.sum` wraps, `Min`/`Max`/`Average` throw on an empty array; floats excluded — NaN semantics differ; quiet inside `query { }`, where the code is a quotation for a provider's translator and the LINQ spelling may not translate)
...
`let mutable total = 0` + `for x in xs do total <- total + x` → `let total = xs |> List.sum` (fix); projections → `sumBy`, general combines → `fold (fun acc x -> ...) init` — same expression, same bindings, no mutable. The module matches the source's resolved kind: measured, `List.sum`/`Array.sum` run LEVEL with the loop while `Seq.sum` is ~50% slower on a list, so this is an idiom rule, and the rewrite never spells `Seq` when it knows better. A combine that appends one element (`acc <- acc @ [ e ]`, `List.append`, `Array.append`, `e :: acc`) is left alone: a fold would keep the copy per element, FR0156 writes the list expression, and FR0051 notes the rest

### FR0051 — performance

`acc <- acc @ [x]` / `acc <- Array.append acc [|x|]` inside a loop copies the accumulator per iteration — O(n²) (note): write the loop as a list expression (FR0156 rewrites a `let mutable` list that is only read after its loops), use a ResizeArray, or cons and `List.rev`. Also `acc <- acc + s` on a STRING (typed-proven) in any loop — the slowest string builder measured, 36x a StringBuilder at 1000 pieces; the note names StringBuilder or collect-then-`String.concat`
...
Weak cryptography (CA5350/5351, note): MD5/SHA1/DES/TripleDES/RC2 construction, TLS certificate-validation bypass via `ServerCertificateValidationCallback`, and broken/deprecated protocol constants (`SecurityProtocolType`/`SslProtocols` Ssl2/Ssl3/Tls/Tls11 — prefer setting nothing and letting the OS negotiate, or Tls12+). Protocols are detected from a CURATED list AND from `[<Obsolete>]`, because the framework marks its own inconsistently: `SslProtocols.Tls11` carries the attribute while `SecurityProtocolType.Tls11` — the one real code sets — does not, a decade after TLS 1.1 was deprecated, so neither signal alone suffices and a later .NET retiring more needs no release from us  Editor alternatives: SHA1 swaps to SHA256/SHA512 (mind persisted hashes), and a weak protocol constant offers three ways out: comment it out of the flags keeping the live protocol beside it, comment the whole setting out and let the OS negotiate, or swap it to `Tls12` (safe even mid-OR-chain, flags-OR being idempotent). Retiring a protocol changes what the WIRE negotiates rather than what callers compile against, so `--api-changes` does not grant it; `{ "FR0065": { "dropLegacyProtocols": true } }` lets an unattended run apply the comment-out, which leaves the diff saying what was retired. Quiet for the WebSocket handshake's SHA-1 when the file spells RFC 6455's GUID, and for a SHA-1 constructed in a match arm whose sibling constructs SHA-256 (a format option the caller chose)

### FR0066 — correctness

SQL assembled from strings (CA2100, note): interpolation holes, `+` chains or `sprintf`/`String.Format` flowing into `CommandText` or a `*Command` constructor — parameterize instead; the text is followed one hop through a `let`, and `CreateCommand(con, text)` and helpers named for SQL (`executeSql`, `runQuery`) count as sinks. Built from VALUES: a hole, operand or argument that is a literal, or a name bound immutably to one (`let schema = "app"`, a `[<Literal>]`) one hop away, is text the author wrote and does not count — `$"SET search_path = \"{schema}\", public"` over a module constant is quiet, there being no parameter to move an identifier into anyway; a `let mutable`, a function parameter or a call stays a value, and so does a name the file binds more than once (a parameter or lambda argument named like a module constant). A module-level text reads its names at module level, never the sink's locals
...
A private/internal record of at most four small struct fields gains `[<Struct>]` (fix), removing a heap allocation per instance — every field is an immutable small struct, so copies are semantically invisible. Stands down where the struct would cost more than the class (CR0081's line): fields over 32 bytes inline (four decimals copy slower than they allocate), an interface the record implements (every call through it boxes), or a value of the record the file boxes — `box`, `:> obj`, `lock`, `ReferenceEquals`, `Unchecked.defaultof`, a parameter typed `obj`, `string`/`hash`, a `%A`/`%O` hole — where the allocation comes straight back (typed; parse-only runs apply the size and interface checks alone). The fix needs the definition to head its decl (`type`, not `and`) with no other attributes (`[<CLIMutable>]` would conflict outright); a PUBLIC record under `--api-changes` stays a note, since `[<Struct>]` there is an ABI and serialization change

### FR0071 — performance

A pure binding inside a `for`/`while`/collection lambda that depends on nothing the loop changes is re-evaluated every iteration; the fix hoists it above the loop, keeping the `#if` it was written under (the directive opens its own line above the anchor). Hoisting moves *n* evaluations to one (and an empty loop still runs it once), so "pure" is a whitelist, not a guess: constants, identifiers, tuples, list literals and core operators that are total on their operands — no calls, no property reads, no `|>`/`>>`/`!`/`:=`, and `/`/`%` only by a non-zero literal that is no signed integral `-1` (`Int32.MinValue / -1` throws OverflowException; a float's `-1.0` is fine). Checked arithmetic throws on overflow, so a file that opens `Checked` (or `Microsoft.FSharp.Core.Operators.Checked`), or an operator the typed tree resolves to the Checked module, stands the rule down. An **array** literal stays put where a list hoists: it is a fresh mutable buffer per iteration, and one shared buffer is not the same program. A `let mutable` the binding reads stands the rule down unless nothing the loop's statement calls can assign it (a local one included — since F# 4.0 a closure captures it as a ref cell)
...
`new` on a non-IDisposable construction is noise — F# convention reserves `new` for disposables (the compiler warns the inverse as FS0760); typed-gated. The `new` STAYS wherever the bare name would resolve to something else: an FSharp.Core conversion function sharing the type abbreviation (`new string(chars, i, n)` becomes `string` applied to a TUPLE and yields "(System.Char[], 1, 12)" — typechecking either way, which is how it reached a shipped id generator), a value declared in the type's own module, a function an `open` in the file brings into scope, a union case, or a name split across namespace fragments
...
`Int32.Parse(s.Substring(6, 5))` → `Int32.Parse(s.AsSpan(6, 5))`, `sb.Append(s.Substring(6, 5))` → `sb.Append(s.AsSpan(6, 5))`, `writer.Write(s.Substring 6)` → `writer.Write(s.AsSpan 6)` (fix — a one-identifier swap). The Substring copy is discarded the moment the consumer has read it; AsSpan reads in place — Parse measured 2.6x and allocation-free, StringBuilder.Append 32 B less per call at time parity. The consumers are those whose span overload is the same operation to the character: the numeric parsers, `StringBuilder.Append`, `TextWriter.Write`/`WriteLine` (StreamWriter and StringWriter included); `StartsWith`/`Equals`/`IndexOf` stay out, their string overloads compare by culture and their span twins ordinally. Fires only when the Substring is DIRECTLY the consumer's argument (no escape), the receiver is typed-proven string, and the compilation actually offers the `ReadOnlySpan<char>` overload on that very type — which is how netstandard2.0/net4x stay untouched with no TFM sniffing, and how a user type's `Append` with a span overload of its own devising is not assumed identical — nor a user type's `Parse`: the parsers must be `System` (or `System.Numerics`) types, whose string and span overloads are one implementation. An F# slice `s[a..]` is left alone: it clamps where AsSpan throws. Framework methods (StartsWith, Contains, interpolation) already run on spans internally and need no rule
...
The canonical `Monitor.Enter x; try body finally Monitor.Exit x` IS F#'s `lock x (fun () -> body)` spelled dangerously — the fix rewrites it (body lines travel verbatim, comments included; the whole released-on-all-paths rule family closes at the source). Gated on single-argument Enter (the `(x, &taken)` overload carries protocol), identical lock text in Enter and Exit, and the typed Monitor entity. A bare `Monitor.Enter` with no try at all is the leak note; a `try/finally` that is the first statement after the Enter (with more following) is recognised as the guard. The same leak under other names is the note too: `SemaphoreSlim.Wait/WaitAsync` without `finally Release`, `ReaderWriterLockSlim.Enter*Lock` without `finally Exit*Lock`, `Mutex.WaitOne` without `finally ReleaseMutex` — an acquire followed by its body with no try/finally (after it or around it) spelling the release on the same receiver; a `use` after the acquire whose binding spells the release, and a `let`/`let!` whose right-hand side or body holds the try (`let! r = async { try ... finally sem.Release() }`, `let mutable acquired = 0` before a counted try), count as the guard; an acquire that ends its block is a wrapper whose caller decides, and an event's `WaitOne` has nothing to release (typed: the receiver is the `Mutex`). Where the release is a statement of the same block the fix follows CR0163: the statements between move under a `try` (re-indented by four) and the release into its `finally`, gated on a release at the acquire's own column ending its line, nothing between acquiring or releasing the receiver again, no binding between unless nothing follows the release (a `let` the try would scope out of what comes after), and no string literal or directive spanning the lines that move; anywhere else the note stands alone
...
A file-private record field `Seen: DateTime` migrates to `DateTimeOffset` in one all-or-nothing edit set (fix) — the instant keeps the clock it was read from, closing the FR0121 class of server-timezone accidents. Strict envelope: every write is `DateTime.UtcNow`/`.Now`/`.MinValue`/`.MaxValue` with Now and UtcNow never mixed (DateTime comparisons ignore Kind — mixing was already a bug, and fixing it silently is still a behavior change); every read is a scalar parity member (`Year`..`Second`, `DayOfWeek`, `Ticks`, `TimeOfDay`...), a same-field comparison, or a same-field subtraction; `Equals`/`CompareTo`/`Subtract` count only with the same field as argument (`Equals(DateTime.MinValue)` is always false on a DateTimeOffset, `CompareTo` throws), and an `Add*` result only where a scalar member reads it at once (`r.Seen.AddDays(1.0).Year`) - one escaping into `string`, a ToString or a return prints and types as a DateTimeOffset. `.Date` (type escapes), `ToString` (format changes) and any unfollowed dataflow bail. OFF BY DEFAULT — a serialization-shape change the repository owner opts into via `"FR0134": true`
...
`TryGetValue` followed by a store on a `ConcurrentDictionary` - `match xs.TryGetValue key with | true, x -> x | false, _ -> let res = compute () in xs.[key] <- res; res` - has a window between the miss and the store for another thread to store first, after which two callers hold two different values for the same key (the indexer is last-writer-wins; `TryAdd |> ignore` hands the loser its own value). The miss arm becomes `xs.GetOrAdd(key, fun _ -> compute ())`, whose add is atomic, so every caller holds the value the dictionary holds; the factory runs outside the dictionary's locks, as the original arm did, so under contention it may still run twice. The `match` and its hit arm stay: a cache takes the hit path almost every time, and there `TryGetValue` is a lock-free read where `GetOrAdd` with a lambda allocates the delegate on every call - measured in benchmarks/PerfClaims on .NET 10 at tier 1, 2.1 ns and nothing against 7.3 ns and 64 B - which is also the shape the .NET documentation recommends for a hot cache. Where running the computation ONCE matters, `GetOrAdd` alone does not buy it - concurrent misses each run the factory and one result is thrown away - and the message says what does: when the factory calls something (`compute ()`, `load key`; not `key * 2`) the fix carries the hint that a `Lazy` value, `xs.GetOrAdd(key, fun _ -> lazy (compute ())).Force()` on a `ConcurrentDictionary<_, Lazy<_>>`, runs it once across concurrent misses, at the price of a `.Force()` per read and FR0152's eviction of a faulted one; and a `Task`, `ValueTask` or `Async` value, where the thrown-away result is a computation already running, gets that note ALONE, since the `Lazy` value changes the dictionary's type and every reader with it. Recognised stores: the indexer (both spellings), `TryAdd(key, res) |> ignore` and `AddOrUpdate(key, res, fun _ _ -> res)`. A miss arm of several `let`s keeps them as the lambda's body, the store line dropped. Declined where the container is any other dictionary (nothing else has `GetOrAdd`), where the value type is a `Lazy` (FR0152's subject once it sits behind `GetOrAdd`) or a function (the lambda would be ambiguous with the plain-value overload), where the key is not a pure atom or a tuple of them spelled the same in the lookup and the store, where the hit arm does anything but return the binder, where the arm reads a mutable local or byref of the enclosing scope (FS0407 inside the lambda), holds a `Span` or takes an address (neither can be captured either) or contains `reraise`, and where the store shares its line with anything else. FR0014, which produces this `TryGetValue` match from a `ContainsKey` check, spells its miss arm `false, _` for the same reason this rule wants it explicit: a bare `_` on a two-case tuple reads as "anything else" and hides what it matches


### FR0155 — performance

`[<Sealed>]` on a class nothing inherits. The JIT treats a class it knows has no subclasses differently in two places, both measured in benchmarks/PerfClaims on .NET 10: a store into a `Node[]` needs no covariance check, since the array cannot really be a `Sub[]` (2.1 to 1.0 ns per element), and `:? Node` is one method-table compare instead of a walk up the hierarchy (0.88 to 0.35 ns per object). A virtual call on a `Node`-typed receiver does not move - dynamic PGO already guards and devirtualizes it - so calls are no reason to seal, and the rule only speaks where one of the two sites that pay exists somewhere in the project: a `Node[]` / `Node array` spelling, or a `:? Node` / `:?> Node` test. Sealing is a promise about the whole program, so the guards are typed and project-wide, and every unknown stands the rule down: the type is a plain class (not abstract, no abstract or default member, not a record, union, struct, interface, exception, delegate or enum, not sealed already); nothing in the project inherits it, read from every entity's base type in the project's typed signature, and no object expression `{ new Node() with ... }` builds on it, read at every typed use of the class across the project's files; no `'T :> Node` constraint names it; the class is internal or private, or the shape-scope gate stands open (a public class of a library is inherited by whoever links it, and an assembly with InternalsVisibleTo opens its internal ones too, so that stands the rule down outright); and the project's typed results are in hand, which an editor may lack. The fix is the attribute on its own line above `type`, below any doc comment and above any attributes already there; a definition in `and` position, or one whose attributes sit on the `type` line itself, keeps the note without the edit

### FR0156 — idiom

A ResizeArray that loops fill one `Add` at a time and the rest of the scope only reads is a list expression: the loops (`for` and `while` alike) move inside the brackets as they are — guards, matches, nested loops, `let`s, the statements around each `Add` and assignments to a mutable of the enclosing scope included, since a list expression compiles to a loop over a collector rather than to a closure and the mutable index a `while` steps stays where it is — and each `Add` becomes the implicit yield of its argument, an argument written on the lines below the `Add` moving up to the call's column. The declaration moves down to the loops; whatever stood between them did not mention the accumulator. `List.ofSeq acc` and `Seq.toList acc` become `acc`, and `for x in acc`, an upcast to `seq` and any FUNCTION whose parameter is `seq<_>` (typed) keep working on the list. A type argument the declaration spelled (`ResizeArray<int64>()`) becomes the list's annotation, since `Add` converted its argument through the method call (an `int` literal to `int64`) where a yield without an expected type would not. Measured in benchmarks/PerfClaims (.NET 10, 1000 ints, two thirds kept): the list expression collects through ListCollector, no growth-doubling reallocations and no final `List.ofSeq` copy, 20% faster on 30% less allocation than the loop it replaces. A drain that wants an ARRAY - an index, `acc.Count` (O(1) on the ResizeArray, a walk on a list), `ToArray()`, `Array.ofSeq`, `Seq.toArray` - stands the rule down rather than write an array expression: its ArrayCollector measured 1.6x the loop's time on the same allocation, and `Array.choose`, the other spelling, the same time on twice the allocation (a `Some` per kept element), so the ResizeArray plus `ToArray()` is the fastest spelling of an array built one element at a time and keeps it; `{ "FR0156": { "arrays": true } }` buys the array expression at that price. Everything else stands the rule down too: a use of the accumulator between the declaration and the loops or inside a loop that is not a statement-position `Add` (a `Count` cap, an `Add` inside a lambda handed to `List.iter` or to a walker's callback, one under a `try`), a loop that also fills another collection, a `let!`, `do!`, `return` or `yield` in the loops, non-consecutive feeding loops, a mutation after the loops, the ResizeArray itself returned, bound or handed to a method or to a function wanting anything but a `seq`, a copy-constructed `ResizeArray(xs)`, a declaration sharing its line with anything else, an element type with subtypes to upcast from (an interface, `obj`, a class without `[<Sealed>]`: `Add` coerces its argument through the method call where a yield fixes the list's type at its first element) or a delegate (`Add` converted a lambda to it where a yield does not), a discarded non-unit statement in the loops (a `d.TryAdd(x, x)` the loop ignored would become a yield of its bool; every other statement must be unit, typed), a loop reading a Span or other byref-like value (the list expression may not capture it), and loops spanning a `#if` or a multi-line literal. `Add` must resolve to `List<'T>.Add` (typed). A loop whose whole body is one `Add` of the loop variable is FR0030's `AddRange`. The `let mutable` LIST form is the same rule with the copy made per element: `let mutable xs = []` (or `List.empty`, annotated or not) fed inside its loops by `xs <- xs @ [ e ]` or `xs <- List.append xs [ e ]` (FSharp.Core's, typed) becomes the list expression with each element as its yield, and by `xs <- e :: xs` when every later read is FSharp.Core's `List.rev xs`, which becomes `xs` (the expression yields in loop order; a list fed at both ends has no order and stays). The result is a list already, so every later read keeps its shape; an assignment after the loops, a `&xs`, a read of `xs` inside its own loops, an element that reads `xs`, or two elements per step stands the rule down, and the annotation is kept on the result. Measured in PerfClaims (1000 ints, two thirds kept): 250x faster on 0.3% of the allocation for the append, 1.5x on half the allocation for the cons-and-reverse. FR0050 leaves the shape to this rule, and FR0051 notes it wherever this rule cannot rewrite it

### FR0157 — idiom

A closed set of string literals matched by name is a union. The proof is a flow analysis over the string SLOTS the matched value passes through — a parameter, a `let`, a record field, a function's return, a name a pattern binds, each plainly `string`, an option of one, or the error of a `Result<_, string>` — connected by the values flowing between them: the set is closed when every source of every slot in the component is a literal (or a constant bound to one, `[<Literal>]` or not) or another slot of the component, and every sink is a match on literals, a comparison with one, a flow into another slot, or a print that goes through `ToString` (an interpolation hole, `string x`, `x.ToString()`; a `%s` hole becomes `%O` and an operand of `+` becomes `string x`). Proven at either end — every call site passing a literal, or a producing function whose every exit (an `Error "..."` for a Result) is one — the value can only ever be one of the literals seen; a set the arms alone spell, with no producer in sight, is a function nothing calls and stands down. The fix inserts `[<RequireQualifiedAccess>] type Name = | Case ...` above the earliest participating declaration in compile order, with the widest visibility of the slots it types - `private` when they are all private, `internal` when one is internal or the component spans files, unmarked only when a public one is among them, so a union the rule adds never widens a library's surface by itself (its name from the matched value's, `Kind` appended where the project already has the type; a case per literal, Pascal-cased from its text or named after the constant it came through, a sentence of more than four spaces kept behind double backticks with its first letter uppercased — ``| ``Bank account could not be selected here`` ``), with `override this.ToString()` returning the original text so a log line or an interpolated string reads exactly as before; rewrites every literal source, arm and comparison to its case; retypes every annotated slot (`string`, `string option`, the error of a `Result`, a record field); and drops a match's catch-all when the proof enumerated every case it could still meet (a bare `_` on an option or a Result also stands for `None` or an `Ok`, so it goes only when that case has an arm of its own) — its comments re-emitted on their own lines rather than deleted — while it stays when the proof found a literal no arm names, whose case it now covers. A variable pattern spelling the name of a module-level string constant beside later arms — `| us -> 2` under `let us = "..."`, which binds a fresh `us` and matches every value — is taken for the comparison the author meant, and the message names it. Multi-file: the call sites and the union may land in other files of the compilation (the editor reports such a finding without the fix), and under `--api-changes` the apply tool's api pass runs the rule with every referencing project's call sites in sight, so an exported function's literal callers in the test project count. An exported function whose callers the host still cannot all see keeps its public signature behind an ADAPTER: its body moves to a private twin typed with the union (`describeCore`), callers in the compilation move to the twin, and a wrapper of the old name and shape maps at the edge under `// TODO: consider changing the API to <Union> and removing this mapping` — a parameter through the union's `OfString`, whose unknown-value arm is the match's own raising catch-all so unknown input fails exactly as before, a return through `string`, `Option.map string` or `Result.mapError string`; a function that is `rec`, `inline`, attributed, used as a value, or whose catch-all returns a default rather than raising, gets no adapter and the rule stands down. Stands down too on a slot fed from anything but a literal or a slot (a call site the host cannot see, a value read from input, a `Result` from a library, a function used as a value or partially applied, a lambda parameter, a member or property, a tuple-bound name, a record pattern, an `Ok` value), a dynamic message (`Error $"failed: {x}"`), a use the union cannot serve (a method call on the string, a comparison with a non-literal, a `%d`-style hole), a use the rule cannot place at all (a field read off a call's result, `(mk b).Kind`, or an assignment to it - unknown is unsafe), a live `| Some null ->` arm like a live `| null ->` one (a dead one goes with the catch-alls), fewer than two distinct literal ARMS in the consumer, a case name F# rejects (a `.` or `/` in a backticked literal, FS0883) or one clashing with a member the union generates or declares (`ToString`, `Tag`, `IsEu` beside `Eu`, FS0023), a record whose field is a slot and which is printed whole or ordered anywhere (`%A`, `string r`, `$"{r}"`, `compare`, `<`, a sort, a Set element or Map key - the field's union type changes its text and its order, cases comparing by declaration), a record a serializer fills (`[<CLIMutable>]`, a `Json`/`DataContract`/`Serializable` attribute, or the record's name as a type argument anywhere — `Deserialize<T>`, `typeof<T>`), a signature file beside any file touched, a name the project already uses for a type with `Kind` appended too, and a wildcard the proof leaves dead but cannot delete whole. A rewrite that retypes a record field is the editor's alone: the print-and-order scan is syntactic and cannot see a generic helper doing either, so a sweep (and the api pass) reports it without a fix. Measured in benchmarks/PerfClaims: the union match runs level with the string match (5.5 against 5.1 ns for four values, runtime-built strings)

### FR0158 — idiom

A `while` that walks a mutable index while a condition holds is a tail-recursive local function — the `while` member of the loop family FR0050 (an accumulator over `for`) and FR0107 (a flag over `for`) cover. `let mutable line = 0` followed, blank lines apart, by `while <condition on line> do line <- line + e` becomes `let rec advanceLine line = if <condition> then advanceLine (line + e) else line` and `let line = advanceLine 0`; `line <- line - e` becomes `retreatLine`. The condition is evaluated once per step in the same order (a side effect in it, `reader.Read()`, runs exactly as often), the value the loop leaves behind is the function's result, and a self tail call compiles back to a loop: measured in benchmarks/PerfClaims level with the loop (27 against 37 ns for 37 steps, no allocation). The declarative spellings were measured and rejected: `Seq.tryFind` over a range doubles the time and allocates 104 B per call, and `Array.tryFindIndex` fits only a condition that indexes one array by the variable. A long condition takes the multi-line `if`. Stands down when the loop body is anything but the one step, the step reads the index, the mutable is assigned anywhere else (it is an immutable `let` after the rewrite), a statement stands between the `let` and the `while`, the initial value or the step spans lines, the range holds a `#if` or a multi-line literal, the loop reads a Span or other byref-like value (typed: the local function could not capture a `ReadOnlySpan<char>` parameter the condition indexes), or the function's name is already in use

### FR0159 — correctness

A floating-point conversion applied to an INTEGER division truncates first: `float (sum / count)` divides as integers, drops the fraction, then widens the whole number the conversion was there to avoid. C# has the twin through an implicit widening (`double avg = sum / count`, CR0168); F# has no implicit conversion, so the shape is the explicit one — `float`/`double`/`float32`/`single`/`decimal` applied to a parenthesised `/` — and the typed check settles it: FSharp.Core's `/` instantiated at an integer type, and FSharp.Core's conversion rather than a shadowing one. The editor offers the rewrite — convert the operands and divide after, `float sum / float count`, parenthesised where the application sat under a tighter operator or as an argument (`float (a / b) ** 2.0`), a negative literal kept in its own parentheses — and a sweep never applies it: an integer division into a floating target is occasionally meant (a page count, whole kilobytes), and the C# twin draws the same line. Quiet under a rounding function (`floor (float (a / b))`, `Math.Truncate`), where the whole number is the point, and after a division by the literal 1. Truncation is likelier still with a literal operand (`float (ms / 1000)` for whole seconds, `float (screenW / 2)` for a pixel centre — a game's screens spell that one ten times over) or a dividend that is a product (`float (x * mw / tw)`, a coordinate scaled by a ratio, a percentage), so those are the editor's alone unless `{ "FR0159": { "all": true } }` asks a sweep for the notes; two bare names, `float (sum / count)`, is the average that lost its fraction and is noted on every run

### FR0160 — correctness

A handler that raises a new exception and drops the one it caught loses the cause: no InnerException, no original stack trace, and the log shows the wrapper alone. `with ex -> raise (ConfigException("bad config"))` becomes `raise (ConfigException("bad config", ex))` where the type has a constructor taking the same parameters plus a trailing `System.Exception` (the typed check compares the parameter shapes, as FR0118 does for its token overloads; the constructor must be reachable from the raise site); a handler binding nothing (`with _ ->`, `with :? IOException ->`) gains the binder in the same fix — `ex`, or `exn`/`inner` where the declaration already uses the name — so `with :? IOException -> raise (X "m")` becomes `with :? IOException as ex -> raise (X("m", ex))`. The gates the C# twin (CR0165) draws: the raise is one of the handler's own statements (through sequences, `if`s, matches, `let` bodies and a computation's `return`; not inside a lambda, an object expression, a local function or a nested try, where it runs in another context); the constructor's arguments read nothing of the caught exception — `AggregateException([| ex |])` carries it, and `ex.Message` in the message is the author's choice; no argument is named (`X(message = "m")`, where a positional `, ex` would not compile). A type without the overload is quiet. `failwith`, `failwithf`, `invalidOp`, `invalidArg` and `nullArg` have no inner-exception spelling; in a handler that never reads what it caught they are noted (`raise (Exception(msg, ex))` keeps the cause), and a `failwithf "...: %s" ex.Message` that read it is the author's decision. CA2200 is the other half of this pair, FR0044 here

### FR0161 — correctness

A mutating method called on the struct a PROPERTY returns mutates a copy: the getter hands out the value, the method changes that, and the stored one never moves — `holder.Counter.Bump()` leaves `holder.Counter.N` at zero. F# refuses the rest of this family at compile time (a method mutating a struct through an immutable `let`, a record field or a `for` variable is FS0256/FS0257), so the property is the one silent case; C# knows the whole family as the mutating-method-on-a-struct-copy defect (CR0161). A mutating method is one this file's own `[<Struct>]` type assigns a field of itself in (`this.N <- ...`), or a BCL struct's known mutator (`MoveNext` on an enumerator struct, `SpinLock.Enter/Exit/TryEnter`, `SpinWait.SpinOnce/Reset`); the typed check settles that the receiver is a property and its type a struct. Note only — copying to a `let mutable`, mutating that and storing it back, or making the type a class, is a design choice

### FR0162 — correctness

Check-then-assign on a shared mutable is a hand-rolled lazy that races. A module-level or `static let` mutable is one slot for the process; `if cache.IsNone then cache <- Some (build ())` lets two threads that both found it empty both run the factory, and a reader between the two stores can see the other's half-built value when the store is not the last thing the factory does. `let private cache = lazy (build ())`, read as `cache.Value`, runs the factory once and is thread-safe by default (ExecutionAndPublication) — FR0154 is the dictionary twin of the same race, CR0164 the C# one. The shape: a module-level or static `let mutable` born empty (`None`, `ValueNone`, `null`, `Unchecked.defaultof`) whose every assignment — a qualified `M.cache <- ...` included — sits under a test of its own emptiness and nothing more: an `if` whose whole condition is `x.IsNone`/`isNull x`/`x = null`/`Option.isNone x`, or the `None`/`null` arm of a `match x with` — a compound condition (`isNull holder || not holder.IsValueCreated || isNull holder.Value`, a reconnecting context holder) re-creates the value on other grounds too, and `lazy` is not the tool for it, so it is quiet. A store inside `lock` (applied or piped, `lock gate <| fun () -> ...`) is guarded and stands the note down; a store elsewhere (a reset) means the value is not lazy-once and `Lazy` is the wrong tool, so the note is withheld too; a `let mutable` local to a function is one thread's, a test file's fixtures likewise, and neither is reported. Note only: the reads move with the binding

### FR0163 — correctness

A `System.Threading.Timer` nothing will reference — `new Timer(tick, null, 0, 1000) |> ignore`, `ignore (new Timer ...)`, `let _ = new Timer ...`, a bare statement, or a `let` the rest of its scope never mentions (nothing stores, returns, passes or disposes it, so the reference dies with the scope: CR0162's shape) — is collectable the moment nothing holds it, and the next garbage collection finalizes it and the callbacks stop; the documentation says so in as many words ("the fact that a Timer is still active does not prevent it from being collected"). F# makes the mistake easier than C# does: FS0020 pushes an unbound construction into `|> ignore`, which reads as tidy. Any mention of the local stands the note down — what leaves the scope is not this rule's to judge. Typed to `System.Threading.Timer` only: a `System.Timers.Timer` is reachable from the timer queue through its own callback while enabled, and survives a collection (measured: an unreferenced Threading.Timer stopped at the first GC, an unreferenced Timers.Timer kept firing). Note only: whether the timer lives in a `use` for the scope, a `let` the caller keeps, or a field, is the design

### FR0164 — correctness

A collection edited inside a `for` loop over itself — `for x in items do if stale x then items.Remove x |> ignore` — throws `InvalidOperationException: Collection was modified` at the next step, and only on the runs where the branch is taken. F# lists are immutable, so the shape is rarer than in C#; a `ResizeArray`, `Dictionary`, `HashSet`, `Queue`, `Stack`, `LinkedList` or `Sorted*` walked by `for` (directly, or through `.Keys`/`.Values`, whose enumerators check the owner's version) and edited in the body throws the same. The fix walks a SNAPSHOT — `for x in Array.ofSeq items do` — so the body may add and remove freely and every element present at the start is visited once, which is what the loop meant. The gates CR0171 draws: the receiver is the enumerated collection by SYMBOL, not a namesake a `let` in the body shadows it with; the edit is one of the body's own statements — inside a lambda, an object expression or a local function it runs later, or not at all, and is not reported; an edit followed at once by `raise`, `failwith`, `return` or `return!` never reaches the next `MoveNext` and is the mutate-and-leave idiom (a `yield` resumes the loop and still counts). Typed-gated to System.Collections.Generic's concrete classes — an `IList<_>`/`IDictionary<_, _>` parameter may be a concurrent collection at run time, and the concurrent collections enumerate a snapshot or tolerate edits by design. A `Dictionary` or `HashSet` allows `Remove`, `Clear` and an overwrite through the indexer mid-enumeration since .NET Core 3.0 (measured on the runtime the tool runs on), so only their additions count — a .NET Framework target would see more; a `List<T>` throws on every edit, an indexer store (`xs.[0] <- v` and `xs[0] <- v` alike) included. A `List<T>` loop that is only `if cond then items.Remove x |> ignore` becomes `items.RemoveAll(fun x -> cond) |> ignore` (CR0171's first fix), unless the condition reads a `let mutable` or a Span, which a closure cannot capture, or the loop holds a comment the whole-loop rewrite would drop: the snapshot then

### FR0165 — correctness

A local-time value compared with a UTC one: `DateTime.Now > startedUtc` where `startedUtc = DateTime.UtcNow`, `DateTime.UtcNow - DateTime.Today`. `Now`/`Today` and `UtcNow` differ by the machine's UTC offset, so the comparison or subtraction flips with the timezone and twice a year with daylight saving — right where it was written, wrong where it runs (CR0169 is the C# twin). A side's kind is read syntactically and confirmed by the typechecker: `DateTime.Now`/`Today` are local, `DateTime.UtcNow` is UTC, `.Date`/`.AddX(...)`/`.Subtract(timespan)` keep the kind of their receiver, and an identifier bound ONCE in this file, immutably (`let startedUtc = DateTime.UtcNow`, module-level or local), carries its right-hand side's kind. The shapes: `=`, `<>`, `<`, `<=`, `>`, `>=` and `-` between two sides of different kinds, `a.CompareTo b`, `a.Equals b`, `a.Subtract b`, `DateTime.Compare(a, b)`. `ToUniversalTime()`, `ToLocalTime()`, `DateTime.SpecifyKind` and `DateTime(..., DateTimeKind.X)` make the kind explicit and stand the rule down; a `let mutable`, a parameter, a field, a name bound twice or from anything unclassifiable is unknown and quiet; `DateTime.Now - DateTime.UtcNow` is the machine's offset and the one subtraction across kinds that is meant. Note only: which kind is the right one is the author's call — `DateTime.UtcNow` throughout, or `.ToUniversalTime()` on the local side

### FR0166 — performance

A prefix or suffix of a string cut out only to be compared with a literal is a `StartsWith`/`EndsWith` that cuts nothing: `s.Substring(0, 6) = "ORDER-"` and `s[..5] = "ORDER-"` → `s.StartsWith("ORDER-", StringComparison.Ordinal)`; `s.Substring(s.Length - 3) = "MED"` and `s[s.Length - 3 ..] = "MED"` → `s.EndsWith("MED", StringComparison.Ordinal)`; `<>` wraps the call in `not (…)`. Measured in benchmarks/PerfClaims: 4.5 → 1.9 ns and 40 → 0 B per comparison. F#'s `=` on strings is ordinal, so `StringComparison.Ordinal` is the same comparison spelled out — the culture-sensitive `StartsWith(string)` is exactly what the rewrite must not emit. The literal's length must equal the cut's (`s.Substring(0, 3) = "ab"` can never hold; a bug, not this rule's). The slice forms are exact on every input under FSharp.Core 5 and later — an F# slice clamps (FS-1077), so a short string gives a shorter slice that is not equal, just as StartsWith answers false — and a sweep applies them; under FSharp.Core 4 a slice throws like a Substring and is held to the same guard. A Substring throws `ArgumentOutOfRangeException` on a short string where StartsWith answers false, so that form is exact only under a length guard in the same condition (`s.Length >= 6 && …`, `if s.Length > 5 then …`, the else branch of `if s.Length < 6`) with no lambda, `let`, match arm or loop rebinding the receiver between the guard and the cut and the receiver an immutable value (never a byref) or a `let mutable` not written between the guard and the cut (a local one by a `<-` or a `&` there, so the `while not (isNull line)` reader loop stays exact; a module or class one also by a call there whose body, defined in this file, assigns it - itself, through the calls it makes, or by a `&` to it, whatever module path the receiver reaches it by, and never another mutable of that name in another scope; any other call is taken as harmless; the rest of an `if` condition after the guard and a loop between the guard and the cut count as between), qualified by modules or read through immutable fields, get-only `member val`s and BCL getters (a `let mutable` may be reassigned between the two, a getter may answer another string), and a sweep applies it only then; without the guard the editor still offers it and the CLI notes. Guards: the receiver is a name or dotted path (nothing evaluated twice), typed-proven string; the `Length` of the suffix forms is the same receiver's; the literal is a plain string constant; the compilation targets a modern framework — String offers `StartsWith(char)`, which netstandard2.0/net4x do not, so a legacy pass stays quiet with no TFM sniffing; no quotation

### FR0167 — performance

`ToCharArray()` copies the whole string into an array the consumer then reads once — a string already is a sequence of its characters: `for c in s.ToCharArray() do` → `for c in s do`; `Array.exists Char.IsDigit (s.ToCharArray())` → `String.exists Char.IsDigit s`; `s.ToCharArray() |> Array.iter f` → `s |> String.iter f` (`iter`, `iteri`, `exists`, `forall` — the four whose String twin returns the same type). Measured in benchmarks/PerfClaims on a runtime-built string: the `for` 13.4 → 9.4 ns and 72 → 0 B, `Array.exists` 9.2 → 4.3 ns and 72 → 0 B; on a string the JIT knows as a frozen constant .NET 10 stack-allocates the copy and the sides measure at parity, while a string that arrives as a parameter pays it. `Array.map`/`filter` change the result type under `String.*` and stay; `Seq.*` over the string was measured 2.3x slower than over the array (the string's CharEnumerator against the array's), so that copy stays too. Guards: the argument-free `ToCharArray()` (the two-argument form slices), directly the loop's source or the Array function's only array argument — bound to a name it may be read twice or mutated and is left alone; `ToCharArray` resolves to System.String's and the Array function to FSharp.Core's; modern framework, as FR0166; single line; no quotation. The `for` throws on a null string on both sides and a sweep applies it; the String module reads a null string as empty (`String.exists f null` is false) where `null.ToCharArray()` threw, so on FSharp.Core 9+ the receiver goes through `nonNull`, which throws the same NullReferenceException — `String.exists f (FSharp.Core.Operators.nonNull s)`, `FSharp.Core.Operators.nonNull s |> String.iter f`, a non-atomic receiver parenthesised, qualified so that no `nonNull` of the project's own (an AutoOpen helper in another file) takes the name — and a sweep applies it; below FSharp.Core 9 or under `--checknulls` (where `nonNull` on a non-nullable `string` warns FS3262), the bare `String.*` rewrites are the editor's offer and a note in a sweep

### FR0168 — performance

`try T.Parse s with <handler> -> fallback` uses the exception as the expected case's signal: a failed parse pays a throw and a stack unwind (tens of microseconds, far more under a debugger) for what `T.TryParse` answers with a bool. The fix: `match T.TryParse s with | true, v -> v | false, _ -> fallback` (`Some v`/`None` and `ValueSome v`/`ValueNone` for a wrapped body), the try gone with its handler. A sweep applies it where the handler covers everything `TryParse` answers false to — a catch-all (`_`, an unread binder, `:? Exception`), or `:? FormatException` plus `:? OverflowException` on a numeric type or `TimeSpan`; a narrower catch (`FormatException` alone around `Int32.Parse`, where an overflow propagated before and would fall back after) is a note. Guards: the body is exactly the one-argument `T.Parse x` of a BCL parse type (`Int32`, `Double`, `Decimal`, `DateTime`, `Guid`, `Boolean`, ...), bare or under `Some`/`ValueSome`; the argument is no shape that throws on its own - an index (`args.[1]`), a cut or a parse of its own (`s.Substring 5`, `Convert.ToInt32 x`), a numeric conversion (`int x`, `Checked.int x`), a `.Value`/`.Head` read anywhere in the chain (`o.Value.Trim()`) of an option, a Nullable, a Lazy or a list - never a KeyValuePair's or a record's - or a downcast (`o :?> string`), or a partial function of FSharp.Core (`List.head`, `Option.get`, `Map.find`, `Map.pick`) - since the handler answered that exception with the fallback too (such a site is a note); a getter, a user call or a `Trim` is taken as a read and the fix stands; every handler answers a plain value (a constant, a name, `None`, an empty literal) and reads nothing of the exception; the null input `Parse` threw for takes the fallback either way (CR0166's line). FR0055 stands down on a try this rule rewrites. CSharp.Refactor's CR0166

### FR0169 — correctness

A `seq<'T>` parameter (annotated or inferred) or a local `let` value typed `seq` enumerated twice on one path — `if Seq.isEmpty xs then ... else Seq.length xs`, a `for x in xs do` followed by `xs |> Seq.sum` — runs the sequence again from the start: what arrived may be a `seq { }` reading a file, a query, a `Seq.map` over a call that does work, and a generator that cannot restart answers nothing the second time. The type is the typechecker's (`seq`/`IEnumerable<'T>` itself; a list or array walks for free; a local built by `Seq.cache`, `Seq.ofList`/`ofArray` or an upcast is cheap to walk again and is skipped); the sites are a `for` over it, a Seq consumer applied to it (`Seq.length xs`, `xs |> Seq.iter f`, a pipeline from it ending in one), or a `List.ofSeq`/`Array.ofSeq`/`Set.ofSeq`/`Map.ofSeq`, matched by symbol (a `let xs = List.ofSeq xs` shadow is another `xs`); a site inside a lambda or a local function is deferred and does not count, and two sites in different arms of one `if`/`match`/`try` are one path each. Note only, at the first site, naming the second's line: materialise once before the first use, or read in one pass. CSharp.Refactor's CR0030

### FR0170 — performance

A loop over a dictionary's keys that looks every key up again — `for k in d.Keys do ... d.[k]` — hashes the key and probes the table once per iteration for a value the enumerator already held: `for KeyValue(k, v) in d do` reads the pair, and every `d.[k]` in the body becomes `v` (fix, all edits together). Guards: `d` is a name or a dotted read typed `Dictionary`, `IDictionary`, `IReadOnlyDictionary` or `SortedDictionary` (never a concurrent one, whose `Keys` is a snapshot where its enumerator is live); the loop variable is a plain name; the body reads the indexer at least once, never stores through it and never rebinds the key (a `d.[k]` under a `let k = ...` reads another key) or the receiver; nothing the rule positively sees write the dictionary runs before a read in the same iteration - the receiver handed to a call, reassigned, rebound or edited, a store through a value of this file bound to it (`let view = d`, `let view, _ = d, 1`, `{ Table = d }`) or through a setter of this file (`h.Bump <- 99`; a store runs the setter, a read the getter), a function, member, getter, active pattern or forced `seq`/`async`/`lazy` value defined in this file whose body touches a dictionary (names one - an auto-property included - stores through one, or calls or forces another such definition, a few levels deep; a `let mutable` hook by every body assigned to it in this file; a read through `ContainsKey`, `Count` or another read-only member is no touch), and in a `seq { }` or `task { }` each `yield`, `let!` and `do!` (an else-less `if` or a loop with a value is an implicit yield, a unit statement is not); a getter counts where it is read, argument or not, every loop nested around a read counts whole, and a read inside a local function keeps the loop - since from .NET Core 3.0 an overwrite or a `Remove` leaves the enumeration running and `v` holds the value from before it (a call's arguments run first, so `use k d.[k]` and `acc.Add(k, d.[k])` convert). A call the rule cannot see into - `log k` defined elsewhere, an interface, a delegate, an event, the BCL - is taken as harmless and the fix stands: the accepted residual is user code behind such a call writing the very dictionary the loop walks; a receiver segment whose getter, defined in this file, builds a dictionary - itself or through a helper of this file - keeps the loop; the value name is `value`, else `v`, else `v1`, whichever the enclosing binding does not already use. CSharp.Refactor's CR0032
...
In C#-style LINQ over `open System.Linq`, a query copied before `Where` or `Select` loads every row and filters it in memory; the query takes the stages and the copy follows them: `db.Orders.ToList().Where(fun o -> o.Total > 0).Select(fun o -> o.Id)` becomes `db.Orders.Where(fun o -> o.Total > 0).Select(fun o -> o.Id).ToList()` (fix). The copy is `ToList`/`ToArray`/`AsEnumerable` on a receiver the typed tree proves is an `IQueryable`. The LINQ chain only, by default: a `query { }` expression shows where the query ends by its shape, and a pipeline (`q |> Seq.toList |> List.filter f`) shows where the rows come into memory just as plainly - the author's visible choice, which the rule leaves be. The `pipelines` knob (default off, `{ "FR0174": { "pipelines": true } }`) takes the pipeline too: `db.Orders |> Seq.toList |> List.filter (fun o -> o.State = 0)` becomes `db.Orders.Where(fun o -> o.State = 0) |> Seq.toList`, for `Seq.toList`/`List.ofSeq` followed by `List.filter`/`List.map` and `Seq.toArray`/`Array.ofSeq` by `Array.filter`/`Array.map`, so the result keeps its type; that rewrite spells `.Where`, so without `open System.Linq` it is a note. `query { }` is never touched. No runtime pair measures it: the win is rows a database never reads or sends. A stage moves only while a provider translates it exactly - a function call, arithmetic or a tuple might not translate (a runtime error) or translate to something else: a filter of `&&`, `||` and `not` over comparisons (`=`, `<>`, `<`, `<=`, `>`, `>=`) and `bool` columns, each comparison a column against a column, a literal, an immutable value or an enum case; a projection of one column; a column a record field of the lambda's parameter, a compiled auto-property (a C# entity's) or a `member val` of the same file, never `[<NotMapped>]`. The first stage that does not qualify stays in memory after the copy. A sweep moves only the comparisons SQL answers as .NET does - integers, `bool`, enums, `Guid`, and `= null`/`<> null`; a string compares under the column's collation (case-insensitive on SQL Server's default), a decimal or a date is rounded to the column's scale, a float is the server's, a nullable column is NULL-unknown where .NET may say true: those the editor offers and a sweep leaves as a note. One nullable comparison is exact: a `Nullable<T>` column of one of the exact types against a value proven non-null (a literal, or a value or enum case whose own type is no Nullable) by `=`, `<`, `<=`, `>`, `>=` under no `not` - SQL's unknown and .NET's false both drop the NULL row, so `o.ParentId = 5` moves in a sweep; under an odd number of `not`s SQL still drops the row where .NET keeps it, and `<>` keeps it in .NET too, so those stay the editor's. F# closures capture only immutable values, so what the lambda reads is the same whenever the query runs. A copy of the same kind right after the stages is the one kept. A column is a VALUE column: a navigation (`o.Customer`) is null in memory without an `Include` where the query joins it, so a null test or a projection of one stays. Moving `ToList()` to the end turns the chain's `IEnumerable<T>` into a `List<T>`: a `let` would carry the new type on, an overload taking `List<T>` would win, `.Reverse()` would bind to `List<T>.Reverse()` - so a sweep moves it only where the value goes into a further Enumerable call (not `Reverse`), a `for ... in`, or a `Seq`/`List`/`Array` function, and the editor offers it elsewhere (`AsEnumerable()` keeps the type and always moves; the pipeline keeps its copy last and its type). Not guarded: with EF Core the copy loaded and tracked every entity, and after the move only the filtered rows (under a projection, none) are tracked - code relying on relationship fix-up or `DbSet.Local` sees less. Declined: a SQLProvider or other provided-type property (not an auto-property: its getter reads the row, and the rule cannot see what the provider translates), a method-call receiver (`ctx.Set<Order>()`), and a comment inside the moved span. C# twin: CR0178.

### FR0173 — performance

A range built only to be mapped over allocates a second collection the size of the result purely to have something to walk: `[| 0 .. dimension - 1 |] |> Array.map f` becomes `Array.init dimension f` (fix). The range is the same length as the result and the same element width, so at 2^30 elements it is four gigabytes of `int` thrown away on top of what was asked for. Measured in benchmarks/PerfClaims (.NET 10, a million elements): 7.0 ms and 8,000,099 B become 3.3 ms and 4,000,044 B — twice as fast on exactly half the allocation, the half being the range. Covers `Array.map`, `List.map` and `Seq.map`, piped (`range |> Array.map f`) or applied (`Array.map f range`), over an array or list range literal whose container matches the module (`Seq.map` takes either), with the mapped function carried over as written. `init` applies the function for 0, 1, … n-1 in that order — the order the map walked the range in — so a side effect in it happens as often and in the same sequence. THE ONE DIFFERENCE is a negative count: `[| 0 .. n - 1 |]` with `n = -1` is the empty range and the map yields `[||]`, where `Array.init -1` raises `ArgumentException`. A count proven non-negative - a non-negative literal, or a `Length`/`Count`/`length` the typed tree says belongs to the FRAMEWORK (a property somebody wrote can answer anything) - is written as is; any other is written `FSharp.Core.Operators.max 0 n` (FSharp.Core's, whatever the project calls `max`), which hands a negative count the empty result the range gave. The rewrite is exact for every count either way, so the sweep applies it too. `0 .. 9` folds to `Array.init 10`; an inclusive bound `[ 0 .. n ]` counts `n + 1` (`List.init (FSharp.Core.Operators.max 0 (n + 1)) f`). The count becomes an argument, so one that is not a single atom keeps its parentheses: `[| 0 .. n * 2 - 1 |]` gives `Array.init (n * 2) f`, because `Array.init n * 2 f` would parse as `(Array.init n) * (2 f)`. Declined: a range that does not start at 0 (the function would have to be shifted), a stepped range, a mapper spanning lines that is not a lambda with its body on a line of its own (a `match` or a first statement after the arrow moves with that line, and the arms or statements aligned to it would fall offside or bind as its arguments), or whose later lines sit left of where the range starts (its text moves as written, lines and columns kept), a `map` that does not resolve to FSharp.Core's — a project with its own `Array` module has no `init` to rewrite to — and a compiler directive or comment inside the expression, whose text the rewrite would drop with the range

### FR0172 — idiom

A match arm that binds a whole list and reads it only by position is a cons pattern: `| itms -> g itms.[0]` becomes `| itmsHead :: _ -> g itmsHead`, with `itms[0]`, `itms.Head`, `List.head itms`, `List.item 0 itms` and their pipelined spellings all standing for the head, `.[1]` / `List.item 1` for the second element, `.Tail` / `List.tail` for the tail — named `itmsTail` where the arm reads it and `_` where it does not (fix); a tail read beside the second element is `itmsHead :: (itmsSecond :: _ as itmsTail)`, the list after the head, since `h :: s :: t` would bind the rest after the second. The edits are spliced in place, so across lines the body must start on a line of its own under a single-line pattern and guard - the pattern grows, and a body continuing below the `->` line would fall offside - no line right under a lengthening edit may be indented past it, no later line may align exactly to a token a shortening edit moves, and a single-line arm may have no line below anchored to text after it. The names come from the arm's own, with a numeric suffix where the file already uses one. Exactness is the compiler's exhaustiveness check: `| itms -> itms.[0]` on an empty list throws `ArgumentException` where `| h :: _ -> h` would fall through with FS0025, so the arm must already be unreachable for the short lists — an earlier unguarded `[]` arm (or `[] \| [_]`, `_ :: []`, a `[ _ ]` of wildcards or names; never `[ 1 ]`, which matches one list, not one length, and never a bare `_` or name: those cover every length, but they cover every array, string and union with it, and the arm is dead code either way) proves length 0 out, and `.[1]` needs length 1 out as well. That arm also settles the type with no check results needed: `[]` matches an F# list and nothing else, so the rule is parse-only, and `[||]` — an array, which indexes in O(1) and has no head/rest pattern — never qualifies. Stands down on any other use of the name (`.Length`, `.IsEmpty`, `.Item`, an index above 1 or a non-literal one, the list passed on whole) and on a rebinding of it inside the arm (a lambda parameter, a `let`, a nested arm, a loop variable). `List.head` and `List.item` are read by name — the rule is parse-only — so a file declaring its own `List` module keeps them, and a lowercase `itms.head` is somebody's extension member rather than the list's `Head`. Measured: no run-time difference between the shapes (`xs.[0]` is `List.nth` walking no cells) — the rule is the idiom, and the runtime exception it turns into a compile-time check
