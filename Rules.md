...
| FR0156 | Idiom | v | | | `let acc = ResizeArray<T>()` filled by `for` loops whose `acc.Add e` are the only thing the loops do with it, then `List.ofSeq acc` | `let acc = [ for ... do ... e ... ]` with the loops moved inside and each `Add` argument as a yield; `List.ofSeq acc` becomes `acc` |
