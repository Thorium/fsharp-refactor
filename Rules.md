...
A loop over a dictionary's keys that looks every key up again — `for k in d.Keys do ... d.[k]` — hashes the key and probes the table once per iteration for a value the enumerator already held: `for KeyValue(k, v) in d do` reads the pair, and every `d.[k]` in the body becomes `v` (fix, all edits together). Guards: `d` is a name or a dotted read typed `Dictionary`, `IDictionary`, `IReadOnlyDictionary` or `SortedDictionary` (never a concurrent one, whose `Keys` is a snapshot where its enumerator is live); the loop variable is a plain name; the body reads the indexer at least once, never stores through it and never rebinds the key (a `d.[k]` under a `let k = ...` reads another key) or the receiver; nothing that may write the dictionary runs before a read in the same iteration - the receiver handed to a call, reassigned or edited, a writing member of another dictionary that may be this one (an alias; one of other type arguments cannot be), a user function, method, getter, setter or constructor, a delegate, a call through an interface, a forced sequence or collection of the user's own, an event's Trigger, an Async runner, a member of a package that merely shares the System namespace, and in a `seq { }` or `task { }` each `yield`, `let!` and `do!`; a getter counts where it is read, argument or not, and every loop nested around a read counts whole - since from .NET Core 3.0 an overwrite or a `Remove` leaves the enumeration running and `v` holds the value from before it (a call's arguments run first, so `use k d.[k]` and `acc.Add(k, d.[k])` convert; a read inside a local function keeps the loop; a local dictionary the function built, with a framework key type and comparer, and never hands on, captures or passes a writer of can be written only by its own mentions, so nothing else counts; a function of this file whose body only computes - FSharp.Core and String calls, no store, no dictionary or sequence named - cannot reach the dictionary either, so `label k` before a read converts; an active pattern of the user's own tried before the read is a call; in a `seq { }` a unit statement yields nothing, an else-less `if` or a loop with a value does); every segment of a dotted receiver is a field, a BCL getter or a `member val`, never a computed property that may hand out another dictionary; the value name is `value`, else `v`, else `v1`, whichever the enclosing binding does not already use. CSharp.Refactor's CR0032
