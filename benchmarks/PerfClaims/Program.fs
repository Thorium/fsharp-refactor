...
// FR0155 (proposed): the three things the JIT does differently for a class
// it knows has no subclasses. An OPEN class is what F# writes by default;
// the sealed twin is byte-for-byte the same code under `[<Sealed>]`.
//   stelem.ref into `C[]` must check the covariance of the array (it might
//   really be a `D[]`) unless C is sealed; `:? C` on a sealed C is one
//   method-table compare instead of a hierarchy walk; a virtual on a
//   receiver statically typed sealed C is a direct call.
type internal OpenNode(v: int) =
    member _.Value = v
    override _.GetHashCode() = v

    override this.Equals(o: obj) =
        match o with
        | :? OpenNode as n -> n.Value = v
        | _ -> false

[<Sealed>]
type internal SealedNode(v: int) =
    member _.Value = v
    override _.GetHashCode() = v

    override this.Equals(o: obj) =
        match o with
        | :? SealedNode as n -> n.Value = v
        | _ -> false

let internal openNodes = Array.init 16 OpenNode
let internal sealedNodes = Array.init 16 SealedNode
let internal openSlots: OpenNode[] = Array.zeroCreate 1000
let internal sealedSlots: SealedNode[] = Array.zeroCreate 1000

// half the objects are the node, half are something else: the miss path
// of `:?` walks the hierarchy for an open class
let openMixed: obj[] =
    Array.init 1000 (fun i -> if i % 2 = 0 then box openNodes.[i % 16] else box (string i))

let sealedMixed: obj[] =
    Array.init 1000 (fun i ->
        if i % 2 = 0 then
            box sealedNodes.[i % 16]
        else
            box (string i))
...
let cases =
    [
        ...
        // FR0155 is PROPOSED, not shipped: these three measure whether
        // `[<Sealed>]` on an internal class is worth a rule at all, on the
        // three sites the rule would trigger on. Measured (.NET 10, x64):
        // the array store halves (2.1 -> 1.0 ns per element) and the type
        // test does better than that (0.88 -> 0.35 ns per object); the
        // virtual call does NOT move — dynamic PGO already guards and
        // devirtualizes it — so a rule should trigger on `C[]` writes and
        // `:? C` tests only, never on member calls
        {
            Code = "FR0155"
            Name = "[<Sealed>]: store into C[] (covariance check)"
            Cat = Perf
            Iters = 20_000
            Before =
                fun () ->
                    for i in 0 .. openSlots.Length - 1 do
                        openSlots.[i] <- openNodes.[i &&& 15]

                    openSlots.[999].Value
            After =
                fun () ->
                    for i in 0 .. sealedSlots.Length - 1 do
                        sealedSlots.[i] <- sealedNodes.[i &&& 15]

                    sealedSlots.[999].Value
        }

        {
            Code = "FR0155"
            Name = "[<Sealed>]: `:? C` over a mixed obj[]"
            Cat = Perf
            Iters = 20_000
            Before =
                fun () ->
                    ...
                    for o in openMixed do
                        match o with
                        | :? OpenNode as n -> acc <- acc + n.Value
                        | _ -> ()
            ...
            After =
                fun () ->
                    ...
                    for o in sealedMixed do
                        match o with
                        | :? SealedNode as n -> acc <- acc + n.Value
                        | _ -> ()
        ...
        {
            Code = "FR0155"
            Name = "[<Sealed>]: virtual call on a C-typed receiver"
            Cat = Perf
            Iters = 20_000
            Before =
                fun () ->
                    ...
                    for i in 0 .. openSlots.Length - 1 do
                        acc <- acc + openNodes.[i &&& 15].GetHashCode()
            ...
            After =
                fun () ->
                    ...
                    for i in 0 .. sealedSlots.Length - 1 do
                        acc <- acc + sealedNodes.[i &&& 15].GetHashCode()
